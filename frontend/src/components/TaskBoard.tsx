...
const TaskBoard: React.FC<TaskBoardProps> = ({ tasks, setTasks, programId }) => {
  ...
  const handleDragStop = (_e: never, _data: never, taskId: string, columnId: 'Todo' | 'In Progress' | 'Done') => {
  ...
  const TaskCard = ({ task }: { task: Task }) => (
    <Draggable
      ...
      axis="y"
      handle=".task-card"
      onStop={(e, data) => handleDragStop(e, data, task.id, task.status as 'Todo' | 'In Progress' | 'Done')}
    >
      <div id={task.id} className="bg-white p-3 rounded shadow task-card">
